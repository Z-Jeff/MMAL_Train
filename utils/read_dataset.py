...
def read_dataset(input_size, batch_size, root, set):
    if set == 'LEAF':
        print('Loading LEAF trainset')
        trainset = dataset.LEAF(input_size=input_size, root=root, is_train=True)
        trainloader = torch.utils.data.DataLoader(trainset, batch_size=batch_size,
                                                  shuffle=True, num_workers=8, drop_last=False)
        print('Loading LEAF testset')
        testset = dataset.LEAF(input_size=input_size, root=root, is_train=False)
